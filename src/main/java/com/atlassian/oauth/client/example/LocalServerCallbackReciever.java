...
/**
 * OAuth 1.0 verification code receiver that runs a Jetty server on a free port, waiting for a
 ...
 * @author Yaniv Inbar
 */
public final class LocalServerCallbackReciever{
  ...
  String code;
  /** Original Token code for validation. */
  String token;
   ...
   * </p>
   */
  public LocalServerCallbackReciever() {
    this("localhost", -1);
  }
   ...
   */
  LocalServerCallbackReciever(String host, int port) {
    this.host = host;
    this.port = port;
  }
  ...
  public String waitForCode(String forToken) throws Exception {
    lock.lock();
    try {
      while (code == null && error == null && forToken!=token) {
  ...
  public static final class Builder {
    ...
    public LocalServerCallbackReciever build() {
      return new LocalServerCallbackReciever(host, port);
    }
  ...
  class CallbackHandler extends AbstractHandler {
    ...
    private void writeLandingHtml(HttpServletResponse response) throws IOException {
      ...
      doc.println("<html>");
      doc.println("<head><title>OAuth 1.0 Authentication Token Recieved</title></head>");
      doc.println("<body>");
